...
export default function PersonDetails() {
  ...
  if (!person) return <div style={{ padding: 16 }}>No person found.</div>;
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: 16 }}>
      <h1 style={{ margin: "0 0 16px" }}>{person.name}</h1>
      ...
      <h2 style={{ margin: "24px 0 12px" }}>Movies</h2>
      {movies.length === 0 ? (
            ...
            return (
              <Link key={m.id} to={`/movie/${m.id}`} className="movie-card" style={{ textDecoration: "none" }}>
                <div className="movie-poster" style={{ position: "relative" }}>
                  {poster ? (
                    <img src={poster} alt={title} style={{ width: "100%", borderRadius: 12 }} loading="lazy" />
                  ) : (
